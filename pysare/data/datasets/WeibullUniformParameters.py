import numpy as np
import pysare as psr
from pysare.data import Dataset
import matplotlib.pyplot as plt


class WeibullUniformParameters():
    def __init__(self, N=300, t_m=3, scale_range=[1, 3], shape_range=[0.5, 5], train_val_split=0.7):
        super(WeibullUniformParameters, self).__init__()
        self.num_features = 2
        self.training_set, self.validation_set = simulate_weibull_uniform_parameters(
            N, t_m, scale_range, shape_range, output_dataset=True).split(train_val_split)
...
def simulate_weibull_uniform_parameters(N, t_m=3, scale_range=[1, 3], shape_range=[0.5, 5], output_dataset=False):
